...
・実行後、--outputPathで指定したディレクトリ内に`{通し番号}_{x座標}_{y座標}_{ズームレベル}.png`が取得したタイルの枚数分生成されます。また、プログラムを実行したディレクトリ内に取得したタイルを繋げた、`input_image{通し番号}.png、target_image{通し番号}.png`が生成されます。
